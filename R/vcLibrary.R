...
vcLibrary = function(package,version,...){
  ...
  for(i in seq_along(package)){
    ...
    if(!dir.exists(vcpd)) stop(paste0("It does not appear that you have done a version control install of '",package[i],"'.\nExpected to find a directory at the location below but did not.\n ",vcpd))

    toLoad = loadVersion(package[i],version[i])

    if(toLoad$Version == "MISSING") stop(paste0("It does not appear that you have done a version control install of version ",toLoad$Version," for '",package[i],"'.\nExpected to find a directory at the location below but did not.\n ",vcpdv))

    vcpdv = paste(vcpd,toLoad$Version,sep=.Platform$file.sep)
    if(!dir.exists(vcpdv)) stop(paste0("It does not appear that you have done a version control install of version ",toLoad$Version," for '",package[i],"'.\nExpected to find a directory at the location below but did not.\n ",vcpdv))
